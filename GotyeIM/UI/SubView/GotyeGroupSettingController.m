...
- (void)viewDidLoad
{
    ...
    [GotyeOCAPI getChatTargetMsgConfig:groupTarget forceRequest:YES];
...
-(void)actionSheet:(UIActionSheet *)actionSheet didDismissWithButtonIndex:(NSInteger)buttonIndex
{
    ...
    else if(actionSheet.tag == 900)
    {
        ...
        int msgConfig = [self groupConfigWithText:text];
        [GotyeOCAPI setChatTarget:groupTarget msgConfig:msgConfig];
...
{
    NSLog(@"onSetGroupMsgConfig. code: %d group: %lld, config: %d", code, group.id, config);
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    switch (indexPath.section) {
        case GroupSettingSectionInfo:
        {
            ...
            UIButton *groupIcon = [[UIButton alloc] initWithFrame:CGRectMake(ScreenWidth - 20 - IconSize, 5, IconSize, IconSize)];
        ...
        case GroupSettingSectionOwner:
        {
            ...
            UILabel *bubbleText = [[UILabel alloc] initWithFrame:CGRectMake(ScreenWidth - 20 - IconSize - 210, 5, 200, IconSize)];
            ...
            UIImageView *ownerHead = [[UIImageView alloc] initWithFrame:CGRectMake(ScreenWidth - 20 - IconSize, 5, IconSize, IconSize)];
        ...
        case GroupSettingSectionMessage:
        {
            ...
            titleStr = @"群消息设置";
            UILabel *accessoryView = [[UILabel alloc]initWithFrame:CGRectMake(ScreenWidth - 20 - 120, 15, 100, 30)];
            accessoryView.text = [self groupMsgConfigDes:[GotyeOCAPI getChatTargetMsgConfig:groupTarget forceRequest:false]];
            accessoryView.textColor = [UIColor grayColor];
            accessoryView.font = [UIFont systemFontOfSize:12];
...
//            break;
        ...
        {
            UIButton *quitButton = [[UIButton alloc] initWithFrame:CGRectMake(20, 8, ScreenWidth - 40, 43)];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    if(indexPath.section == GroupSettingSectionInfo && [[GotyeOCAPI getLoginUser].name isEqualToString:groupTarget.ownerAccount])
    {
        ...
        [self.navigationController pushViewController:viewController animated:YES];
    } else if (indexPath.section == GroupSettingSectionMessage) {
        UIActionSheet *sheet = [[UIActionSheet alloc]initWithTitle:@"选择群消息提醒设置" delegate:self cancelButtonTitle:@"取消" destructiveButtonTitle:nil otherButtonTitles:@"接收消息并提醒", @"接收消息但不提醒", @"屏蔽群消息", nil];
        sheet.tag = 100;
...
NSString *shielding = @"屏蔽群消息";
NSString *receiving = @"接收消息但不提醒";
NSString *receivingAndNotifying = @"接收消息并提醒";

- (NSString *)groupMsgConfigDes:(int)config
{
    switch (config) {
        case ShieldingGroupMsg:
        case NotifyingGroupMsg:
            return shielding;
        case ReceivingGroupMsg:
            return receiving;
        case ReceivingGroupMsg | NotifyingGroupMsg:
        default:
            return receivingAndNotifying;
    }
}

- (int)groupConfigWithText:(NSString *)config
{
    if ([config isEqualToString:shielding]) {
        return ShieldingGroupMsg;
    }
    
    if ([config isEqualToString:receiving]) {
        return ReceivingGroupMsg;
    }
    
    if ([config isEqualToString:receivingAndNotifying]) {
        return ReceivingGroupMsg | NotifyingGroupMsg;
    }
...
- (void)viewDidLoad
{
    ...
    [GotyeOCAPI getChatTargetMsgConfig:groupTarget forceRequest:true];
}
